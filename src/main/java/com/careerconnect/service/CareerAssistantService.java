...
/**
 * Builds safe prompts and sends them to the LLM through the ChatClient interface.
 * This service is responsible for:
 * - Retrieving verified data from repositories
 * - Constructing context-aware prompts
 * - Making sure the LLM gets only what it needs (no extra data)
 */
@Service
public class CareerAssistantService {
    ...
    // base system prompt that goes with every request
    private static final String BASE_SYSTEM_PROMPT = """
            You are an advisory campus placement assistant for CareerConnect AI.
            Your role is to help students with placement-related questions.
            Important rules:
            - You are ADVISORY only. You cannot create, approve, reject, or modify applications.
            - Only use the information provided in the context. Do not make up data.
            - If asked to perform actions like approving applications, politely explain that you can only provide guidance.
    ...
    /**
     * Handles a chat message. Depending on available context (studentId, driveId),
     * it builds different prompts.
     */
    public String handleChat(String studentId, String driveId, String userMessage) {
        StringBuilder contextBuilder = new StringBuilder();

        // if we have a student ID, fetch their profile for context
        ...
        // if we have a drive ID, fetch drive details for context
        ...
        // if both student and drive exist, run eligibility and include result
